...
//print_r( $view_args );
//print_r( $donations);
//$status = charitable_get_donation( $donation->ID )->get_status_label();
//print_r( $status );
	...
	<table class="charitable-my-donations charitable-table">
		...
		<tbody>
			...
			<tr>
				...
				?>
				<td>
				<?php if( charitable_get_donation( $donation->ID )->get_status_label() == "Paid"   ){
					?> 
					<a href="<?php echo esc_url( charitable_get_permalink( 'donation_receipt_page', array( 'donation_id' => $donation->ID ) ) ); ?>"><?php _e( 'View Receipt', 'charitable' ); ?></a></td> 
